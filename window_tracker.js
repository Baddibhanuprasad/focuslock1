...
class WindowTracker {
  ...
  async classifyContent(windowInfo) {
    ...
    // YouTube regular watch pages are monitored by caption analysis in the browser extension
    // Don't classify them as distracting here - let the extension handle it
    const isYouTubeRegular = (title.includes('youtube') || proc.includes('youtube')) 
      && !title.includes('shorts') && !title.includes('music.youtube');
    if (isYouTubeRegular) {
      return 'neutral';
    }
